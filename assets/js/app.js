...
Hooks.HighlightText = {
  mounted() {
    ...
    const highlightedCodeArea = document.querySelector(
      `#${this.el.id} pre code`,
    );

    if (name && highlightedCodeArea) {
      highlightedCodeArea.className = highlightedCodeArea.className.replace(
        /language-\S+/g,
        "",
      );
...
Hooks.UpdateLineNumber = {
  mounted() {
    ...
    if (this.el.type == "textarea") {
      lineNumberElement = document.querySelector("#line-number");
      this.el.addEventListener("input", () => {
        this.updateLineNumber(this.el.value, lineNumberElement);
      });
      ...
      const lineId = this.el.getAttribute("data-line");
      lineNumberElement = document.querySelector(`#${lineId}`);
      this.updateLineNumber(this.el.textContent, lineNumberElement);
    }
  },
  updateLineNumber(value, lineNumberElement) {
    if (!lineNumberElement) return;
...
function insertTabCharacter(elem) {
  ...
  elem.value = `${value.substring(0, selectionEnd)}\t${value.substring(
    selectionEnd,
  )}`;
